...
const Reserva = () => {
    ...
    // Configuración del plano de asientos
    const rows = 15; // 15 filas
    const seatsPerRow = 10; // 10 asientos por fila
    const [availableSeats, setAvailableSeats] = useState(() => {
        // Generar un array de asientos disponibles
        return Array.from({ length: rows }, (_, rowIndex) =>
            Array.from({ length: seatsPerRow }, (_, seatIndex) => `Row ${rowIndex + 1} Seat ${seatIndex + 1}`)
        );
    });
    ...
    return (
        <div className="reserva-container">
            ...
            {/* Selección de Asientos - Plano del Cine */}
            <div className="reserva-field">
